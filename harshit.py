...
def gconnect():
        ...
        if login_session['email'] == 'harshitga@gmail.com':
                login_session['admin'] = 1
                json.dumps("admin")
        else:
                login_session['admin'] = 0
        #cheks if user is already in user database. If not it stores user
info in User database.
...
100px;-webkit-border-radius: 100px;-moz-border-radius:
...
def index():
        ...
        best = session.query(Product).join(Bestseller).filter(Product.id ==
Bestseller.product_id)
        if 'username' not in login_session:
                return render_template('index.html',best=best, events=events,
chaf=chaf, table=table, house=house, counter=counter, tab=tab,
catimg=catimg,flag=1,log=0)
        else:
                if login_session['admin']==0:
                        return render_template('index.html',best=best, events=events,
chaf=chaf, table=table, house=house, counter=counter, tab=tab,
catimg=catimg,flag=1,log=1)
                else:
                        return render_template('indexadmin.html',best=best, events=events,
chaf=chaf, table=table, house=house, counter=counter, tab=tab,
catimg=catimg,flag=1,log=2)




# Shows all restuarants if user is not logged in else shows
restaurants created by the user.
...
def productdetail(productid):
        ...
        related = session.query(Product).filter_by(catagory =
show.catagory).filter(Product.id != show.id).limit(4)
        if 'username' not in login_session:
                return render_template('CD-601.html', chaf=chaf, table=table,
house=house, counter=counter, tab=tab, show=show, typ=typ,
related=related, flag=2,log=0)
        else:
                if login_session['admin']==0:
                        return render_template('CD-601.html', chaf=chaf, table=table,
house=house, counter=counter, tab=tab, show=show, typ=typ,
related=related, flag=2,log=1)
                else:
                        return render_template('CD-601.html', chaf=chaf, table=table,
house=house, counter=counter, tab=tab, show=show, typ=typ,
related=related, flag=2,log=2)
...
def productedit(productid):
        ...
        if request.method == 'POST':
                ...
                related = session.query(Product).filter_by(catagory =
editedItem.catagory).filter(Product.id != editedItem.id).limit(4)
                return render_template(
                        'editproduct.html', productid=productid, item=editedItem,
chaf=chaf, table=table, house=house, counter=counter, tab=tab,
typ=typ, related=related, flag=2,log=2)
...
def catalogupdate():
        ...
        return render_template('listview.html', table=table, house=house,
counter=counter, tab=tab, show=show,flag=4,log=2)
...
def addtocart(productid):
        ...
        if request.method == 'POST':
                ...
                if exist.count():
                        ...
                        amt= item.price * qty
                        newcart = Cart(id=cid, user_id=us, product_id=productid,
...
def showcart(userid):
        ...
        return render_template('showcart.html', table=table, house=house,
counter=counter, tab=tab, show=show,flag=5,log=1)
...
def eventeditform(eid):
        ...
        if request.method == 'POST':
                ...
                tab = session.query(Product).filter_by(catagory="TB")
                return render_template(
                        'editevent.html', item=editedItem, chaf=chaf, table=table,
house=house, counter=counter, tab=tab, flag=6,log=2)
...
def allproductcat(productcat):
        ...
        if 'username' not in login_session:
                return render_template('counters.html', chaf=chaf, table=table,
house=house, counter=counter, tab=tab, show=show,
typ=typ,flag=2,log=0)
        else:
                if login_session['admin']==0:
                        return render_template('counters.html', chaf=chaf, table=table,
house=house, counter=counter, tab=tab, show=show,
typ=typ,flag=2,log=1)
                else:
                        return render_template('counters.html', chaf=chaf, table=table,
house=house, counter=counter, tab=tab,productcat=productcat,
show=show, typ=typ,flag=2,log=2)
...
def payment():
        ...
        if request.method == 'POST':
                ...
                return render_template('pay1.html', total=total, flag=2, log=2)
