...
def index():
	...
	if 'username' not in login_session:
		...
		if login_session['admin']==0:
			return render_template('index.html',best=best, events=events, chaf=chaf, table=table, house=house, counter=counter, tab=tab, catimg=catimg,flag=1,log=1, cartid=login_session['user_id'])
		else:
			return render_template('indexadmin.html',best=best, events=events, chaf=chaf, table=table, house=house, counter=counter, tab=tab, catimg=catimg,flag=1,log=2, cartid=login_session['user_id'])
...
def productdetail(productid):
	...
	if 'username' not in login_session:
		...
		if login_session['admin']==0:
			return render_template('CD-601.html', chaf=chaf, table=table, house=house, counter=counter, tab=tab, show=show, typ=typ, related=related, flag=2,log=1, cartid=login_session['user_id'])
		else:
			return render_template('CD-601.html', chaf=chaf, table=table, house=house, counter=counter, tab=tab, show=show, typ=typ, related=related, flag=2,log=2, cartid=login_session['user_id'])
...
def productedit(productid):
	...
	if request.method == 'POST':
		...
		return render_template(
			'editproduct.html', productid=productid, item=editedItem, chaf=chaf, table=table, house=house, counter=counter, tab=tab, typ=typ, related=related, flag=2,log=2, cartid=login_session['user_id'])
...
def productcreate(productcat):
	...
	if request.method == 'POST':
		...
		counter = session.query(Product).filter_by(catagory="CN")
		tab = session.query(Product).filter_by(catagory="TB")
		return render_template('additem.html', chaf=chaf, table=table, house=house, counter=counter, tab=tab, productcat=productcat, flag=2,log=2, cartid=login_session['user_id'])
...
def catalogupdate():
	...
	return render_template('listview.html', table=table, house=house, counter=counter, tab=tab, show=show,flag=4,log=2, cartid=login_session['user_id'])
...
def showcart(userid):
	...
	return render_template('showcart.html', table=table, house=house, counter=counter, tab=tab, show=show,flag=5,log=1, cartid=login_session['user_id'])

@app.route('/openfile/<string:filename>')
def openfile(filename):
	...
	tab = session.query(Product).filter_by(catagory="TB")
	if 'username' not in login_session:
		return render_template(filename, chaf=chaf, table=table, house=house, counter=counter, tab=tab, flag=7,log=0)
	else:
		if login_session['admin']==0:
			return render_template( filename, chaf=chaf, table=table, house=house, counter=counter, tab=tab, flag=7,log=1)
		else:
			return render_template( filename, chaf=chaf, table=table, house=house, counter=counter, tab=tab, flag=7,log=2)
...
def eventeditform(eid):
	...
	if request.method == 'POST':
		...
		tab = session.query(Product).filter_by(catagory="TB")
		return render_template(
			'editevent.html', item=editedItem, chaf=chaf, table=table, house=house, counter=counter, tab=tab, flag=6,log=2, cartid=login_session['user_id'])
...
def allproductcat(productcat):
	...
	if 'username' not in login_session:
		...
		if login_session['admin']==0:
			return render_template('counters.html', chaf=chaf, table=table, house=house, counter=counter, tab=tab, show=show, typ=typ,flag=2,log=1, cartid=login_session['user_id'])
		else:
			return render_template('counters.html', chaf=chaf, table=table, house=house, counter=counter, tab=tab,productcat=productcat, show=show, typ=typ,flag=2,log=2, cartid=login_session['user_id'])
...
def payment():
        ...
        if request.method == 'POST':
                ...
                return render_template('pay1.html', total=total, flag=2, log=1, cartid=login_session['user_id'])
